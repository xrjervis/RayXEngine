...
Terrain::Terrain() {
	m_mesh = std::make_unique<Mesh<VertexPCUTBN>>(PRIMITIVE_TYPE_TRIANGLELIST, true);
}
...
void Terrain::GenerateFromHeightMap(u32 gridWidth, u32 gridHeight, float scale, const Image& heightMap, const Image& colorMap) {
	...
	for (u32 h = 0; h < gridHeight; ++h) {
		for (u32 w = 0; w < gridWidth; ++w) {
			...
			m_mesh->PushVertex(VertexPCUTBN(blPos, blColor, blUV, blNormal, blTangent, blBitangent));
			m_mesh->PushVertex(VertexPCUTBN(brPos, brColor, brUV, brNormal, brTangent, brBitangent));
			m_mesh->PushVertex(VertexPCUTBN(tlPos, tlColor, tlUV, tlNormal, tlTangent, tlBitangent));
			m_mesh->PushVertex(VertexPCUTBN(trPos, trColor, trUV, trNormal, trTangent, trBitangent));
