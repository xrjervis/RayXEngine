...
RHIOutput::RHIOutput(RHIDevice* device, const std::wstring& windowTitle, eWindowMode mode, float aspect)
	: m_device(device) {
	CreateOutputWindow(windowTitle, mode, aspect);
	CreateSwapChain();
	CreateRenderTarget();
...
void RHIOutput::CreateSwapChain() {
	...
	swapChainDesc.Width = (UINT)m_window->GetClientWidth();
	swapChainDesc.Height = (UINT)m_window->GetClientHeight();
...
void RHIOutput::CreateOutputWindow(const std::wstring& windowTitle, eWindowMode mode, float aspect) {
	m_window = std::make_unique<Window>(windowTitle, mode, aspect);
}
...
void RHIOutput::CreateDepthStencilTarget() {
	...
	texDefn.m_dimension = Vector2(m_window->GetClientWidth(), m_window->GetClientHeight());
